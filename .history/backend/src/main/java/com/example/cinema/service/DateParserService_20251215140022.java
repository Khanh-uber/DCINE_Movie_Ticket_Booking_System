...
public class DateParserService {
    // Bắt ngày dạng "16 12" (sau normalize từ 16/12)
    private static final Pattern DATE_DD_MM_SPACE =
        Pattern.compile("\\b(\\d{1,2})\\s+(\\d{1,2})\\b");
    ...
    public LocalDate parse(String text) {
        ...
        if (norm.contains("ngay kia") || text.contains("mốt"))
        ...
        // ====== 4.5) NGÀY DÍNH TRONG TIME (vd: "toi 16 12") ======
        Matcher spacedDate = DATE_DD_MM_SPACE.matcher(norm);
        if (spacedDate.find()) {
            int d = Integer.parseInt(spacedDate.group(1));
            int m = Integer.parseInt(spacedDate.group(2));
            return safe(d, m, LocalDate.now().getYear());
    ...
    private DayOfWeek parseDayOfWeek(String norm) {
        ...
        if (norm.contains("chu nhat") || norm.contains("cn"))
