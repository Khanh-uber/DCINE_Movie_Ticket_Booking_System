    ...
    public TheaterDTO findTheater(String theaterText, String locationText) {
        ...
        if (normLocationText != null || !normLocationText.isBlank()){
            for (Map<String, Object> m : allLocation){
                if (normLocationText.equals(normalize((String) m.get("city_name")))){
        ...
        if (locationId != null) {
    for (com.example.cinema.entity.Theater t : all) {

        String theaterName = normalize(t.getName());
        String locName = normalize(t.getLocation().getName());

        if (theaterName.contains(normTheater)
                && locName.contains(normLocationText)) {
