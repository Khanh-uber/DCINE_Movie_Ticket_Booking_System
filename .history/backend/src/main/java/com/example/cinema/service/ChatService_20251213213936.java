...
public class ChatService {
    ...
    public ChatService(LLMService llmService, ShowtimeService showtimeService, MovieService movieService, ReplyHelper replyHelper, LLMReplyService replyService, MovieLookupService movieLookupService,
        DateParserService dateParserService, TheaterLookupService theaterLookupService, FormatShowtimes formatShowtimes, ObjectMapper objectMapper, ShowTimeRepository showTimeRepo, TimeParserService timeParserService) {
    ...
    public ChatResponse handleFullShowtime(IntentResult.Entities e) {
        ...
        // CHUẨN HÓA GIỜ CHIẾU
        // =====================================================
        TimeFilter timeFilter = timeParserService.parse(e.getTime());
        ...
        String instruction = """
            ...
            YÊU CẦU NỘI DUNG:
            1. Mở đầu nhiệt tình: Xác nhận tên phim (từ Context.movie) và ngày chiếu (từ Context.date) bằng giọng điệu phấn khích. Ví dụ: "Phim này đang cháy vé!"
            2. TRÌNH BÀY: Liệt kê Lịch chiếu phim theo từng rạp (từ Context.showtimes_by_theater). Mỗi rạp là một đầu mục, các suất chiếu được liệt kê bên dưới.
            3. Kêu gọi hành động: Kết thúc bằng lời rủ rê và gợi ý LỌC hoặc XEM TIẾP.
            - Gợi ý: "Chọn rạp bạn muốn quẩy ngay!"
        """;
    ...
    public ChatResponse handleMovieShowtime(IntentResult.Entities e) {
        String movieText = e.getMovie();
        String dateText = e.getDate();
        String timeText = safe(e.getTime());
        String theaterText = e.getTheater();
        // 1) Thiếu movie hỏi lại
        if (movieText == null || movieText.isBlank()) {

            String instruction = """
                Người dùng muốn hỏi GIỜ CHIẾU nhưng chưa nói tên phim.
                Hỏi lại ngắn gọn: "Bạn muốn xem giờ chiếu phim nào?"
            """;
        ...
        // 5) parse time
        TimeFilter timeFilter = timeParserService.parse(timeText);
...
YÊU CẦU NỘI DUNG:
1. Mở đầu nhiệt tình: Xác nhận tên phim (Context.movie) và ngày (Context.date) bằng giọng điệu phấn khích.
2. XỬ LÝ LỌC:
    - Nếu Context.time CÓ giá trị: Phải diễn đạt rõ ràng rằng đây là kết quả ĐÃ ĐƯỢC LỌC theo buổi/giờ đó. Ví dụ: "Tui đã lọc suất [Sáng] cho bạn rồi nè!"
    - Nếu Context.theater CÓ giá trị: Nhắc tên rạp đã lọc.
3. TRÌNH BÀY: Liệt kê các GIỜ CHIẾU theo từng rạp từ Context.showtimes_by_theater. Mỗi rạp là một đầu mục, liệt kê các suất chiếu (bao gồm định dạng) bên dưới.
4. Kêu gọi hành động: Kết thúc bằng lời rủ rê, gợi ý tiếp tục hành động (chọn rạp/đổi buổi/đổi ngày).
    - Ví dụ: "Giờ cưng chọn rạp nào để quẩy đây?" HOẶC "Hay là bạn muốn lọc lại suất tối, hoặc xem ngày khác nhé?"
        """;
