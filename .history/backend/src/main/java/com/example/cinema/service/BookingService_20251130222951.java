...
public class BookingService {
    ...
    public BookingResponse createBooking(Long showtimeId, Long accountId, BookingRequest req){
        List<Booking> pendingSeat = bookingRepo.findAllPendingByAccountId(accountId);
        for (Booking b : pendingSeat) {
        Long id = b.getBookingId();
        bookingSeatRepo.deleteSeatsByBookingId(id);
        bookingRepo.deletePendingBookingById(id); // dùng query có điều kiện status = 'PENDING'
}
