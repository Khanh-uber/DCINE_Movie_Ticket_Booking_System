...
public class ChatService {
    ...
    public ChatResponse handleGioChieuPhim(IntentResult.Entities e) {
        String movieText = e.getMovie();
        String dateText = e.getDate();
        String timeText = e.getTime();
        String theaterText = e.getTheater();
        ...
        for (Map<String, Object> row : rows) {
            ...
            Object startObj = row.get("startAt");
            if (startObj instanceof Timestamp ts) {
                dto.setStartAt(ts.toLocalDateTime);
            } else if (startObj instanceof LocalDateTime ldt) {
                dto.setStartAt(ldt);
            }
        ...
        String instruction = """
            Bạn là trợ lý D-Cine, trả lời rõ ràng, dễ đọc.
            1) Xác nhận phim (Context.movie) và ngày (Context.date).
            2) Nếu Context.time có giá trị: diễn đạt là đang lọc theo buổi/giờ đó.
            3) Liệt kê các GIỜ CHIẾU theo từng rạp từ Context.showtimes_by_theater.
            4) Kết thúc bằng câu gợi ý: chọn rạp/đổi buổi/đổi ngày để xem tiếp.
        """;
