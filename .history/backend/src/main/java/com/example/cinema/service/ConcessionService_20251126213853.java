...
public class ConcessionService {
    ...
    public List<ConcessionResponse.SeatItems> calculate(List<BookingResponse.Item> items){
        List<ConcessionResponse.SeatItems> resultItems = new ArrayList<>();
        for (BookingResponse.Item it : items){
            ConcessionResponse.SeatItems seatItems = new ConcessionResponse.SeatItems();
            seatItems.setCode(it.getCode());
            seatItems.setZone(it.getZone());
            seatItems.setPrice(it.getPrice());
            seatItems.setType(it.getType());
            resultItems.add(seatItems);
        }
        return resultItems;
    ...
    public ConcessionResponse loadSummary(){
        ...
        if (booking != null){
            ...
            List<ConcessionResponse.SeatItems> seatItems = calculate(List<BookingResponse.Item)
