...
public class ConcessionService {
    ...
    public ConcessionResponse updateCart(ConcessionCartRequest req){
        ...
        for(ConcessionCartRequest.CartItem it : req.getItems()){
            ...
            Double basePrice = (Double) (row.get("price")).DoubleValue()
            ...
            Long priceDiff = 0L;
            if (variantRow != null) {
                priceDiff = ((Number) variantRow.get("price_diff")).DoubleValue();
            }
             // 3) Tính unit price (giá theo variant)
            Long unitPrice = basePrice + priceDiff;

            // 4) Tính total price
            Long totalPrice = unitPrice * it.getQty();
