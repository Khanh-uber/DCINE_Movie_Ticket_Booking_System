    ...
    public ConcessionResponse updateCart(ConcessionCartRequest req){
        ...
        if (req != null && req.getItems() != null) {
            for(ConcessionCartRequest.CartItem it : req.getItems()){
                ...
                dto.setCode((String) row.get(comboItems));
                dto.setTitle(combo.getTitle());
                dto.setImageUrl(combo.getComboUrl()); // tuỳ tên field getter trong entity

                dto.setVariant(variantValue);
                dto.setVariantLabel(variantLabel != null ? variantLabel : (variantValue != null ? variantValue : ""));
