...
public class BookingService {
    ...
    public BookingResponse createBooking(Long showtimeId, Long accountId, BookingRequest req){
        List<Booking> list = bookingRepo.findAllPendingByAccountId(accountId);
        System.out.println(">>> PENDING bookings: " + list.size());

        for (Booking b : list) {
            System.out.println("XÓA booking " + b.getBookingId());
        }
        // Booking pendingSeat = bookingRepo.findPendingBookingByAccountId(accountId);
        // bookingSeatRepo.deleteSeatsByBookingId(pendingSeat.getBookingId());
        // bookingRepo.deleteBookingById(pendingSeat.getBookingId());
