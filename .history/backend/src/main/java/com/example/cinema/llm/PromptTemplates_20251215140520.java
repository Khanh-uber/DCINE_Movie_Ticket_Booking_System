...
- KHÔNG được chọn "hoi_phim_dang_chieu"
  khi entity movie != null
2. hoi_phim_sap_chieu → hỏi phim sắp chiếu
3. hoi_phim_hot → hỏi phim nổi bật/hot
4. hoi_the_loai → hỏi phim theo thể loại
5. hoi_danh_gia → chỉ dùng khi người dùng hỏi ĐÁNH GIÁ / CHẤT LƯỢNG phim
Chỉ chọn intent này khi câu hỏi chứa các cụm:
- "hay không", "dở không", "có đáng xem không"
- "xem được không", "ổn không"
- "phim này hay chứ", "phim này có hay không"
Ví dụ đúng:
- "Phim này có hay không?"
- "Phim đó xem được không?"
...
- Nếu câu có ngày ở dạng số (dd/mm, dd-mm):
  → BẮT BUỘC gán vào date_phrase
  → Không được gán vào time_phrase dù đứng sau từ "tối", "sáng"
...
QUY TẮC TÁCH DATE & TIME (BẮT BUỘC):

- Nếu trong câu có cụm gồm:
  [buổi trong ngày] + [ngày cụ thể]
...
THÌ PHẢI:
- time_phrase = chỉ phần buổi
  (tối / sáng / chiều / khuya)
- date_phrase = chỉ phần ngày
  (16/12, 15-11, 2/9)
==========================
...
//   }
// }
User: "Cuối tuần này phim phiên chợ của quỷ chiếu như thế nào?"
JSON:
{
  "intent": "hoi_co_chieu",
  "entities": {
    "movie": "phiên chợ của quỷ",
    "genre": [],
    "theater": null,
    "time_phrase": null,
    "date_phrase": "cuối tuần này",
    "mood": [],
    "location": null
  }
}
