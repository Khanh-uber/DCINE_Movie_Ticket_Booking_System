...
public class ChatService {
    ...
    private ChatResponse handleNowShowing(){
        List<MovieD> movies = movieService.getNowShowingMovies();
        return new ChatResponse("Cac phim đang chiếu hiện tại",movies);
    }
    private ChatResponse handleUpcoming(){
        var movies = movieService.getComingSoonMovies();
        return new ChatResponse("Cac phim sắp chiếu",movies);
    }
