...
public class SeatMapService {
    ...
    for (Map.Entry<String, String> entry : seatTypes.entrySet()) {
        String groupedRows = entry.getKey();   // ví dụ: "A,B,C"
        String zoneName    = entry.getValue(); // ví dụ: "Standard"

        // tách từng row
        String[] rows = groupedRows.split(",");

        // xem row hiện tại có nằm trong nhóm không
        for (String r : rows) {
            if (r.trim().equalsIgnoreCase(row)) {
                return zoneName.toLowerCase();  // trả về standard / vip / couple
            }
        }
    }

    // không match → default (hiếm khi xảy ra)
    return "standard";
}
    public SeatMapResponse getSeatMap(Long showtimeId){
        ...
        for (Seat s : seats){
            ...
            String zone = null;
            for (String key : seatTypes.keySet()) {
                String[] rows = key.split(",");
                for (String r : rows) {
                    if (r.trim().equals(row)) {
                        zone = seatTypes.get(key);
                        break;
                    }
                }
            }
