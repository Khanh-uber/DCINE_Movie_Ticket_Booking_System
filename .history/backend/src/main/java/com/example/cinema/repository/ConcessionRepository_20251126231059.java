...
public interface ConcessionRepository extends JpaRepository<BookingSeat, BookingSeatKey>{ 
    ...
    Set<String> findBookedSeats(@Param("showtimeId") Long showtimeId);
        ...
        // (ConcessionService)
        @Query(value="""
                        select distinct concat(s.row_label, s.seat_number) as code, bs.price_at_booking, st.name, st.price_multiplier, s2.base_price from seat s
