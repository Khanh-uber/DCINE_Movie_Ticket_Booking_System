...
public class ChatService {
    ...
    List<String> rawGenres = e.getGenre(); 
    List<String> normalizedGenres = genreNormalizer.normalizeGenres(rawGenres);
    ...
    List<MovieDTO> movies = movieService.findMoviesByGenres(normalizedGenres);

    String context;
    try {
        context = objectMapper.writeValueAsString(movies);
    } catch (Exception ex) {
        context = "[]";
    }
    ...
    String genreText = String.join(", ", rawGenres);

    String instruction = """
        Đây là các phim thuộc thể loại người dùng yêu cầu: %s.
        Hãy trả lời:
        - Ngắn gọn (1–2 câu), đùa cợt, ăn chơi một chút.
