...
public class ChatService {
    ...
    public ChatService(LLMService llmService, ShowtimeService showtimeService, MovieService movieService, ReplyHelper replyHelper, LLMReplyService replyService, MovieLookupService movieLookupService,
        DateParserService dateParserService, TheaterLookupService theaterLookupService, FormatShowtimes formatShowtimes, ObjectMapper objectMapper) {
    ...
    public ChatResponse handleFullShowtime(IntentResult.Entities e) {
        ...
        GIỌNG ĐIỆU:
- Bắt buộc dùng giọng thân mật, đùa cợt, ăn chơi (cưng, lên kèo, xõa, cháy).
- Sử dụng emoji thất vọng nhẹ (😥, 💔, hoặc 😉 để rủ rê).

NỘI DUNG:
1. Thông báo kết quả: Nhắc lại Phim và Ngày đã yêu cầu từ Context và xác nhận rằng KHÔNG có suất chiếu nào.
2. Tuyệt đối KHÔNG DÙNG từ "Xin lỗi", thay bằng cụm từ thể hiện sự tiếc nuối thân mật ("Ái chà", "Tiếc quá", "Hơi buồn nha").
3. Kêu gọi hành động: Gợi ý người dùng thử ngay các mốc thời gian khác (ngày mai, cuối tuần) hoặc một bộ phim đang hot khác.
4. Tôn trọng dữ liệu: Đảm bảo sử dụng tên phim và ngày chính xác từ Context.
