...
public class HoldSeatService {
    ...
    public void processHoldAction(Long showtimeId,Long accountId ,List<HoldSeatRequest.SeatItem> seats, String action) {
        ...
        if ("hold".equalsIgnoreCase(action)) {
            ...
            redisSeatService.holdForUser(showtimeId, accountId, seats);
        }
        else if ("release".equalsIgnoreCase(action)) {

            // Trả ghế lại cho user hiện tại
            redisSeatService.releaseForUser(showtimeId, accountId, seats);
        }
        else {
