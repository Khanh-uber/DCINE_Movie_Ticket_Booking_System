...
public class ChatService {
    ...
    public ChatService(LLMService llmService, ShowtimeService showtimeService, MovieService movieService, ReplyHelper replyHelper, LLMReplyService replyService, MovieLookupService movieLookupService, DateParserService dateParserService ) {
        ...
        this.formatShowtimes = new FormatShowtimes();
    }
    ...
    public ChatResponse handleTheaterShowtime(IntentResult.Entities e) {
        ...
        TheaterDTO theaterDTO = theaterLookupService.findTheater(theaterText);
        if (theaterDTO == null) {
            // khong tim thay rap
            String instruction = """
                Không tìm thấy rạp người dùng yêu cầu trong hệ thống.
                Hãy trả lời:
                - Xin lỗi vì không tìm thấy rạp.
        ...
        if (theater == null) {
        String ctx = safeJson(mapOf(
                "theater_input", theaterText
        ));
        ...
        String reply = replyService.reply("hoi_suat_theo_rap", ctx, instruction);
        return new ChatResponse(reply, null);
    }
