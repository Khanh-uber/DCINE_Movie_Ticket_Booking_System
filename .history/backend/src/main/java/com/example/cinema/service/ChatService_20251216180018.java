...
public class ChatService {
    ...
    private boolean isHardIntentChange(String currentIntent, String lastIntent) {
        // Lần đầu chat → không reset
        if (lastIntent == null) return false;

        // Nhóm intent liên quan PHIM / LỊCH CHIẾU
        boolean lastIsMovie = isMovieIntent(lastIntent);
        boolean currentIsMovie = isMovieIntent(currentIntent);

        // Nếu 1 cái là movie, 1 cái không → reset
        return lastIsMovie != currentIsMovie;
    }
    private boolean isMovieIntent(String intent) {
    ...
    public ChatResponse handleTheaterShowtime(IntentResult.Entities e) {
        ...
        LocalDate date = DateParserService.
    ...
    public ChatResponse handleFullShowtime(IntentResult.Entities e) {
        ...
        // =====================================================
        System.out.println("theater chua chuan hoa:"+theaterText);
        ...
        System.out.println("theater da chuan hoa:"+theater.getName());
    ...
    public ChatResponse handleMessage(String message, String memoryKey) {
        ...
        if (e.getDate_phrase() == null) e.setDate_phrase(ctx.getDatePhrase());
        if (e.getTime_phrase() == null) e.setTime_phrase(ctx.getTimePhrase());
        if (e.getLocation() == null) e.setLocation(ctx.getLocation());
        ...
        switch (intentName) {
            ...
            case "hoi_gio_chieu_phim":
                res =  handleMovieShowtime(e);
                break;

            case "hoi_suat_theo_rap":
                res =  handleTheaterShowtime(e);
                break;
        ...
        // 4️⃣ ⭐ UPDATE STATE SAU KHI HANDLE
        if (e.getMovie() != null) ctx.setMovie(e.getMovie());
        if (e.getTheater() != null) ctx.setTheater(e.getTheater());
        if (e.getDate_phrase() != null) ctx.setDatePhrase(e.getDate_phrase());
        if (e.getTime_phrase() != null) ctx.setTimePhrase(e.getTime_phrase());
        if (e.getLocation() != null) ctx.setLocation(e.getLocation());
